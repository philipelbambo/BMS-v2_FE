...
const BoardinghouseLanding: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="relative">
            <div className="relative h-[700px] mb-8 rounded overflow-hidden shadow-2xl bg-white">
              ...
              <button
                onClick={prevSlide}
                className="absolute left-6 top-[250px] -translate-y-1/2 bg-white/90 hover:bg-[#001F3D] text-gray-900 hover:text-white p-4 rounded shadow-xl transition-all z-10"
              >
                <ChevronLeft className="w-8 h-8" />
              </button>
              <button
                onClick={nextSlide}
                className="absolute right-6 top-[250px] -translate-y-1/2 bg-white/90 hover:bg-[#001F3D] text-gray-900 hover:text-white p-4 rounded shadow-xl transition-all z-10"
              >
      ...
      {/* Benefits Section - With Working Video */}
      <section id="benefits" className="py-24 bg-gray-50">
        ...
        <div className="w-full relative overflow-hidden rounded-lg shadow-xl">
          <div className="relative">
            <video
              src="/videos/Boardinghouse.mp4" // ✅ CORRECT PATH
              autoPlay
              muted
              loop
              playsInline
              className="w-full h-[600px] object-cover"
              poster="https://via.placeholder.com/1920x600/001F3D/FFFFFF?text=Real-Time+Updates+You+Can+Trust"
              onError={(e) => {
                console.error("Video failed to load:", e);
                // Fallback to an image if video fails
                const videoElement = e.target as HTMLVideoElement;
                videoElement.style.display = 'none';
                const fallbackImage = videoElement.parentElement?.querySelector('.video-fallback');
            ...
            <img 
              src="https://images.unsplash.com/photo-1555854877-bab0e564b8d5?w=1920&h=600&fit=crop&auto=format&auto=compress" 
              alt="Boardinghouse showcase"
              className="w-full h-[600px] object-cover hidden video-fallback"
            />
          </div>

          {/* Overlay with meaningful content */}
          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white px-6 sm:px-10">
            <h3 className="text-3xl sm:text-4xl font-bold text-center mb-4">Real-Time Updates You Can Trust</h3>
            <p className="text-lg text-center max-w-3xl text-gray-200">
              Our system syncs instantly — see room availability, new listings, and booking confirmations in real time.
            </p>
          </div>
        </div>
      </section>
